...
//	}
	...
	private void swim(int k) {
		Key swimmer = pq[k];
		Key lesser = less(pq[k / 2], swimmer);

		while (k > 1 && lesser != swimmer) {
			pq[k] = lesser;
			k = k / 2;
			lesser = less(pq[k / 2], swimmer);
		}
		pq[k] = swimmer;
	}
...
//	}
	...
	private Key less(Key i, Key j) {
		if (i == null) return j;
		if (j == null) return i;
		if (comparator == null) {
			if (((Comparable<Key>) i).compareTo(j) < 0) {
				return i;
			}
		} 
		else {
	...
	/*
	 * private Key lesser(int i, int j) { if (comparator == null) { if
	 * (((Comparable<Key>) pq[i]).compareTo(pq[j]) < 0) { return pq[i]; } return
	 * null; } else { if (comparator.compare(pq[i], pq[j]) < 0) { return pq[i];
	 * } return null; } }
	 */
	 ...
	 */
	/*
	 * public static void main(String[] args) { MaxPQ<String> pq = new
	 * MaxPQ<String>(); while (!StdIn.isEmpty()) { String item =
	 * StdIn.readString(); if (!item.equals("-")) pq.insert(item); else if
	 * (!pq.isEmpty()) StdOut.print(pq.delMax() + " "); } StdOut.println("(" +
	 * pq.size() + " left on pq)"); }
	 */
